...
/* let count = 0;
setInterval(()=>{
  document.querySelector('.first').style.transform = `translate(${Math.sin(++count) * 10 }px, ${(++count) * 5}px)`
},100) */

// 최소, 최대 값 사이 난수 반환 함수
/* function getRandomMinMax(min, max) {
  if (min > max) {
    alert('최소값은 최대값보다 작아야합니다.');
    return;
  }
